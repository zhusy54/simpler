...
bool create_scheduler_state(
    ...
    for (int64_t task_id = 0; task_id < total_tasks; ++task_id) {
        ...
        if (active_subtasks == 0 || logical_block_num > UINT16_MAX / active_subtasks) {
            LOG_ERROR(
        ...
        const auto *predicate =
            reinterpret_cast<const SchedulerDispatchPredicate *>(payload + SCHEDULER_GRAPH_PREDICATE_OFFSET);
        if (!scheduler_dispatch_predicate_metadata_valid(predicate->addr, predicate->elem_size, predicate->op)) {
            LOG_ERROR("A5 HBG AICore scheduler: task id=%" PRId64 " has malformed predicate metadata", task_id);
            return false;
        }
        ...
        if (slot.task_attrs.requires_sync_start() && logical_block_num == 1) {
            LOG_ERROR("A5 HBG AICore scheduler: sync-start task id=%" PRId64 " must have block_num > 1", task_id);
            return false;
        }
        if (!scheduler_resident_v0_task_shape_supported(
                active_subtasks, logical_block_num, slot.task_attrs.requires_sync_start()
            )) {
            if (legacy_shape_task_id < 0) legacy_shape_task_id = task_id;
        }
    ...
    run_control->aic_task_count = aic_task_count;
...
extern "C" int validate_runtime_impl(Runtime *runtime, const HostApi *api, int execution_rc) {
    ...
    SchedulerStateOwner scheduler_owner{};
    {
        std::scoped_lock lock(scheduler_state_owners_mutex);
        auto it = scheduler_state_owners.find(runtime);
        if (it != scheduler_state_owners.end()) scheduler_owner = it->second;
    }
    if (scheduler_owner.state_base != nullptr && execution_rc == 0) {
        const AicoreSchedulerLayout &scheduler_layout = scheduler_owner.layout;
        const uint64_t scheduler_state_size = scheduler_layout.total_size;
        ...
        int scheduler_state_rc = api->copy_from_device(
            host_scheduler_state, scheduler_owner.state_base, static_cast<size_t>(scheduler_state_size)
        );
        if (scheduler_state_rc != 0) {
            ...
            const auto *control =
                scheduler_state_at<SchedulerRunControl>(host_scheduler_state, scheduler_layout.run_control_offset);
            const auto *contexts = scheduler_state_at<SchedulerWorkerContext>(
                host_scheduler_state, scheduler_layout.worker_contexts_offset
            );
            const auto *task_controls =
                scheduler_state_at<SchedulerTaskControl>(host_scheduler_state, scheduler_layout.task_controls_offset);
            const auto *completion_inboxes = scheduler_state_at<SchedulerCompletionInbox>(
                host_scheduler_state, scheduler_layout.completion_inboxes_offset
            );
            const auto *ready_inboxes =
                scheduler_state_at<SchedulerReadyInbox>(host_scheduler_state, scheduler_layout.ready_inboxes_offset);
            const auto *ready_directory = scheduler_state_at<SchedulerReadyDirectory>(
                host_scheduler_state, scheduler_layout.ready_directory_offset
            );
            const auto *dispatch_slots =
                scheduler_state_at<SchedulerDispatchSlot>(host_scheduler_state, scheduler_layout.dispatch_slots_offset);
            ...
            uint64_t ready_contention_giveups = 0;
            ...
            const uint64_t executable_task_count = scheduler_layout.executable_task_count;
            const uint64_t executable_subtask_count = scheduler_layout.executable_subtask_count;
            const uint64_t ordinary_task_count = executable_task_count - scheduler_layout.gang_task_count;
            for (int32_t i = 0; i < runtime->worker_count; ++i) {
                ...
                ready_contention_giveups += contexts[i].ready_contention_giveup_count;
            ...
            for (uint32_t type = 0; type < SCHEDULER_CORE_TYPE_COUNT && ready_inboxes_empty; ++type) {
                for (uint64_t inbox = 0; inbox < control->scheduler_count; ++inbox) {
            ...
            if (control->scheduler_error != 0 ||
                ...
                control->bootstrap_scan_arrived_count != control->scheduler_count ||
                control->bootstrap_scan_complete == 0 || control->bootstrap_arrived_count != control->scheduler_count ||
                ...
                if (control->scheduler_error != 0) {
                    LOG_ERROR(
                        ...
                        " type=%" PRIu64 " graph_tasks=%" PRIu64 " storage=0x%" PRIx64 " reserved=0x%" PRIx64
                        " window_last_index=0x%" PRIx64 " site=%" PRIu64,
                        control->error_task_id, control->scheduler_error, control->error_core_id,
                        control->error_core_type, control->error_graph_task_count, control->error_storage_address,
                        control->error_reserved_address, control->error_task_window_last_index, control->error_site
                    );
                }
                rc = -1;
                ...
                LOG_INFO(
                    ...
                    " ready_cas_retries=%" PRIu64 " ready_contention_giveups=%" PRIu64 " state_polls=%" PRIu64
                    ...
                    " completion_enqueues=%" PRIu64 " completion_resolves=%" PRIu64 " ready_to_kernel_cycles=%" PRIu64
                    ...
                    ready_contention_giveups, state_polls, fanin_loads, wake_registers, wake_cas_retries,
