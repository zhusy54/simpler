...
inline __aicore__ __gm__ SchedulerGangCommand *scheduler_gang_command_at(
    ...
    return scheduler_state_at<SchedulerGangCommand>(
        scheduler_state_base, context->gang_commands_offset + scheduler_index * sizeof(SchedulerGangCommand)
    );
}
...
inline __aicore__ void scheduler_gang_publish_command(
    ...
    __gm__ SchedulerGangCommand *command = scheduler_gang_command_at(scheduler_state_base, scheduler, scheduler_index);
...
inline __aicore__ bool scheduler_gang_update_subtree_token(
    ...
    __gm__ SchedulerGangParticipant *participant =
        scheduler_gang_participant_at(scheduler_state_base, scheduler, cohort_index, scheduler_index);
    ...
    for (uint32_t child_slot = 0; child_slot < 2; ++child_slot) {
        ...
        __gm__ SchedulerGangParticipant *child =
            scheduler_gang_participant_at(scheduler_state_base, scheduler, cohort_index, children[child_slot]);
...
inline __aicore__ bool scheduler_gang_root_token_ready(
    ...
    __gm__ SchedulerGangParticipant *root =
        scheduler_gang_participant_at(scheduler_state_base, scheduler, cohort_index, 0);
...
inline __aicore__ bool scheduler_fill_explicit_dispatch_slot(
    ...
    slot->ready_inbox_index = scheduler->scheduler_index;
    ...
    if (status != SchedulerGraphResult::OK) {
        scheduler_record_error(
            run_control, task_id, status, &graph, scheduler, SchedulerErrorSite::GANG_MATERIALIZE_FAILED
        );
        return false;
    }
...
inline __aicore__ bool scheduler_gang_fill_single(
    ...
    const int32_t pending_slot = scheduler_gang_find_free_pending_slot(scheduler_state_base, scheduler, worker_id);
...
inline __aicore__ bool scheduler_gang_fill_mix_block(
    ...
    for (uint32_t candidate = 0; candidate < SCHEDULER_PENDING_SLOT_COUNT; ++candidate) {
        ...
        for (uint8_t subtask_slot = 0; subtask_slot < 3; ++subtask_slot) {
            ...
            __gm__ SchedulerDispatchSlot *slot =
                scheduler_dispatch_slot_at(scheduler_state_base, scheduler, worker_id, candidate);
            if (scheduler_dispatch_state(scheduler_gm_query(slot->publication)) != SchedulerDispatchSlotState::FREE) {
                all_free = false;
                break;
            }
...
inline __aicore__ int64_t scheduler_gang_select_ready_task(
    __gm__ void *scheduler_state_base, __gm__ SchedulerWorkerContext *scheduler, uint64_t priority_bit
) {
    for (uint64_t task_id = 0; task_id < scheduler->graph_task_count; ++task_id) {
...
inline __aicore__ bool scheduler_gang_admit_one(
    ...
    __gm__ SchedulerGangCoordinator *coordinator = scheduler_gang_coordinator_at(scheduler_state_base, scheduler);
    ...
    for (uint32_t candidate = 0; candidate < SCHEDULER_GANG_COHORT_COUNT; ++candidate) {
        __gm__ SchedulerGangCohort *cohort = scheduler_gang_cohort_at(scheduler_state_base, scheduler, candidate);
    ...
    const int64_t task_id = scheduler_gang_select_ready_task(scheduler_state_base, scheduler, priority_bit);
    if (task_id < 0) {
        scheduler_gm_fetch_and(coordinator->ready_priority_bits, ~priority_bit);
        if (scheduler_gang_select_ready_task(scheduler_state_base, scheduler, priority_bit) >= 0)
    ...
    const uint32_t participant_count = required_participants < scheduler->scheduler_count ?
    ...
    for (uint32_t participant_index = 0; participant_index < participant_count; ++participant_index) {
        __gm__ SchedulerGangParticipant *participant =
            scheduler_gang_participant_at(scheduler_state_base, scheduler, cohort_index, participant_index);
        ...
        if (single_aiv) {
            participant->block_stride = static_cast<uint32_t>(scheduler->scheduler_count * 2);
            ...
            participant->block_stride = static_cast<uint32_t>(scheduler->scheduler_count);
    ...
    __gm__ SchedulerGangCohort *cohort = scheduler_gang_cohort_at(scheduler_state_base, scheduler, cohort_index);
    ...
    scheduler_publish_cache_line(&coordinator->admitted_count);
    scheduler_gang_publish_command(
        scheduler_state_base, scheduler, 0, cohort_index, generation,
        scheduler_task_requires_sync_start(metadata->flags) ? SchedulerGangCohortState::DRAINING :
                                                              SchedulerGangCohortState::DISPATCHING
    );
    return true;
...
inline __aicore__ bool scheduler_gang_service_participant(
    ...
    bool progress = state != SchedulerGangCohortState::RETIRING &&
                    scheduler_gang_forward_command(
    ...
    if (state == SchedulerGangCohortState::DRAINING) {
        if (scheduler_gang_local_slots_drained(scheduler_state_base, scheduler, participant->active_mask)) {
            progress =
        ...
        scheduler_gang_release_local_slots(scheduler_state_base, scheduler, cohort_index, cohort_generation);
        progress =
        ...
        scheduler_gang_publish_retire(scheduler_state_base, scheduler, participant, cohort_index, cohort_generation);
    }
...
inline __aicore__ bool scheduler_gang_service_owner(
    ...
    if (scheduler->scheduler_index != 0 || cohort->participant_count == 0) return false;
    ...
    if (state == SchedulerGangCohortState::DRAINING &&
        scheduler_gang_root_token_ready(
            scheduler_state_base, scheduler, cohort_index, generation, SchedulerGangTokenPhase::DRAIN
        )) {
        cohort->drain_complete_cycles = scheduler_cycles();
        cohort->state = static_cast<uint64_t>(SchedulerGangCohortState::STAGING);
        scheduler_publish_cache_line(cohort);
        scheduler_gang_publish_command(
            scheduler_state_base, scheduler, 0, cohort_index, generation, SchedulerGangCohortState::STAGING
        );
        progress = true;
    } else if (state == SchedulerGangCohortState::STAGING &&
               scheduler_gang_root_token_ready(
                   scheduler_state_base, scheduler, cohort_index, generation, SchedulerGangTokenPhase::STAGE
               )) {
        cohort->stage_complete_cycles = scheduler_cycles();
        cohort->state = static_cast<uint64_t>(SchedulerGangCohortState::RELEASING);
        scheduler_publish_cache_line(cohort);
        scheduler_gang_publish_command(
            scheduler_state_base, scheduler, 0, cohort_index, generation, SchedulerGangCohortState::RELEASING
        );
        progress = true;
    } else if ((state == SchedulerGangCohortState::RELEASING || state == SchedulerGangCohortState::DISPATCHING) &&
               scheduler_gang_root_token_ready(
                   scheduler_state_base, scheduler, cohort_index, generation, SchedulerGangTokenPhase::DISPATCH
               )) {
        ...
        scheduler_publish_cache_line(&coordinator->active_dispatch_cohort);
        scheduler_gang_publish_command(
            scheduler_state_base, scheduler, 0, cohort_index, generation, SchedulerGangCohortState::EXECUTING
        );
        progress = true;
    }
    ...
    if (state == SchedulerGangCohortState::EXECUTING &&
        scheduler_gang_root_token_ready(
            scheduler_state_base, scheduler, cohort_index, generation, SchedulerGangTokenPhase::COMPLETION
        )) {
        __gm__ SchedulerTaskControl *control =
        ...
        if (!scheduler_resolve_completion(
                ...
                completion_stats, owner_state, profiling_level, false, nullptr
            )) {
            return false;
        }
        ...
        scheduler_publish_cache_line(cohort);
        scheduler_gang_publish_command(
            scheduler_state_base, scheduler, 0, cohort_index, generation, SchedulerGangCohortState::FREE
        );
        __gm__ SchedulerGangParticipant *root =
            scheduler_gang_participant_at(scheduler_state_base, scheduler, cohort_index, 0);
        scheduler_observe_cache_line(root);
        (void)scheduler_gang_forward_command(
            scheduler_state_base, scheduler, root, cohort_index, generation, SchedulerGangCohortState::FREE
        );
        scheduler_gang_publish_retire(scheduler_state_base, scheduler, root, cohort_index, generation);
        progress = true;
    }
    state = static_cast<SchedulerGangCohortState>(cohort->state);
    if (state == SchedulerGangCohortState::RETIRING &&
        scheduler_gang_root_token_ready(
            scheduler_state_base, scheduler, cohort_index, scheduler_gang_retire_token(generation),
            SchedulerGangTokenPhase::COMPLETION
        )) {
        cohort->task_id = SCHEDULER_TASK_ID_INVALID;
        cohort->state = static_cast<uint64_t>(SchedulerGangCohortState::FREE);
        scheduler_publish_cache_line(cohort);
        // A gang task is globally resolved only after every Scheduler has observed
...
inline __aicore__ bool scheduler_service_gang(
    ...
    bool progress = false;
    const bool owner = scheduler->scheduler_index == 0;
    ...
    if (!owner) {
        __gm__ SchedulerGangCommand *command =
            scheduler_gang_command_at(scheduler_state_base, scheduler, scheduler->scheduler_index);
    ...
    for (uint32_t cohort_index = 0; cohort_index < SCHEDULER_GANG_COHORT_COUNT; ++cohort_index) {
        ...
        if (owner) {
            cohort = scheduler_gang_cohort_at(scheduler_state_base, scheduler, cohort_index);
            scheduler_observe_cache_line(cohort);
    ...
    if (owner) progress = scheduler_gang_admit_one(graph, scheduler_state_base, scheduler, run_control) || progress;
    return progress;
}
