...
struct LegacyAicpuExecutor {
    // ===== Thread management state =====
    std::atomic<int32_t> thread_idx_{0};
    std::atomic<bool> init_done_{false};
    std::atomic<bool> init_failed_{false};

    // Parallel-handshake coordination (see LegacyAicpuExecutor::init). hs_setup_done_
    ...
    // Parallel-boot-classify coordination (see LegacyAicpuExecutor::run). classify_ready_
...
static LegacyAicpuExecutor g_aicpu_executor;

// ===== LegacyAicpuExecutor Method Implementations =====

int32_t LegacyAicpuExecutor::init(Runtime *runtime) {
    if (runtime == nullptr) {
    ...
    if (is_leader) {
        LOG_INFO("LegacyAicpuExecutor: Initializing");
    ...
    if (is_leader) {
        ...
        init_done_.store(true, std::memory_order_release);
        LOG_INFO("LegacyAicpuExecutor: Init complete");
    } else {
 ...
 */
int32_t LegacyAicpuExecutor::run(Runtime *runtime) {
...
void LegacyAicpuExecutor::deinit(Runtime *runtime) {
    ...
    LOG_INFO("DeInit: LegacyAicpuExecutor reset complete");
}

// ===== Public Entry Point =====

extern "C" int32_t legacy_aicpu_prewarm_callable(Runtime *runtime) {
