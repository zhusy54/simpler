...
int32_t AicoreLifecycle::post_handshake_init(Runtime *runtime) {
    ...
    auto *coordinator = scheduler_state_at<SchedulerCohortCoordinator>(
        scheduler_state_base, bootstrap_context->cohort_coordinator_offset
    );
    const uint64_t executable_task_count = run_control->expected_task_count - run_control->inline_completed_count;
    const bool has_cohort_tasks = coordinator->cohort_task_count != 0;
    ...
    if (has_cohort_tasks) required_clusters = static_cast<uint64_t>(aic_count);
    const int32_t active_clusters = static_cast<int32_t>(std::min<uint64_t>(required_clusters, aic_count));
    if ((run_control->aic_worker_demand > static_cast<uint64_t>(aic_count) && !has_cohort_tasks) ||
        (run_control->aiv_worker_demand > static_cast<uint64_t>(aiv_count) && !has_cohort_tasks) ||
    ...
    for (int32_t cluster = 0; cluster < aic_count; ++cluster) {
        ...
        for (int32_t lane = 0; lane < PLATFORM_CORES_PER_BLOCKDIM; ++lane) {
            ...
            const bool active_lane =
                ...
                (has_cohort_tasks || scheduler_lane || (lane == 0 && cluster < requested_aic) || additional_aiv_lane);
