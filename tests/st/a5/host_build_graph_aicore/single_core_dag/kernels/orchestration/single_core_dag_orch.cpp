...
PTO2TaskId submit_task(
    const ChipTensor &task_state, int64_t core_type, int64_t logical_id, const PTO2TaskId *deps, int32_t dep_count
) {
    CoreTaskArgs task;
    ...
    return (core_type == 0 ? rt_submit_aic_task(kAicFuncId, task) : rt_submit_aiv_task(kAivFuncId, task)).task_id();
}
