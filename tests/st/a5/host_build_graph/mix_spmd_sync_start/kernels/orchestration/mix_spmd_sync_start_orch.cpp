...
PTO2TaskId submit(
    const ChipTensor &output, const MixedKernels &kernels, int16_t block_num, int64_t base_cache_line, bool sync_start,
    const PTO2TaskId *dependency = nullptr
) {
    CoreTaskArgs args;
...
void report_layout(const ChipTensor &layout, int32_t task, int16_t block_num, int32_t base_cache_line, int32_t mask) {
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipTaskArgs &args) {
    const ChipTensor &output = args.tensor(0).ref();
    const ChipTensor &layout = args.tensor(1).ref();
    ...
    PTO2TaskId root = submit(output, aic, block_nums[0], base_cache_line, false);
